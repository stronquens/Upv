...
public class Recomendacion {
    ...
    public ReferenciasWeb maxReferencias() {
        // COMPLETAR
        ReferenciasWeb pweb = null;
        if (!this.qp.esVacia()){
            pweb = this.qp.recuperarMin();
            this.qp.eliminarMin();
            this.noPags--;
        }
        return pweb;
    ...
    public ReferenciasWeb[] maxKReferencias(int k) {
        ...
        ReferenciasWeb[] res = null;
        if (!this.qp.esVacia()){
            // Tamanyo de k o del numero de elementos en qp
            if (k < this.noPags){
                res = new ReferenciasWeb[k];
            } else {
                res = new ReferenciasWeb[this.noPags];
            }
            int contador = 0;
            while(!this.qp.esVacia() && contador < k) {
                res[contador] = maxReferencias();
                contador++;
            }
        }
        return res;
