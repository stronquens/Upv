...
public class GrafoMunicipios {
    ...
    protected Map<Integer, Municipio> ver2Muni; 
    protected Map<Municipio, Integer> muni2Ver;
    ...
    public GrafoMunicipios() {
        ...
        ver2Muni = new TablaHash<Integer, Municipio>(NUM_MAX_MUNICIPIOS);
        muni2Ver = new TablaHash<Municipio, Integer>(NUM_MAX_MUNICIPIOS);
        try {         
    ...
    public int obtenerCodigo(Municipio m) {
        //COMPLETAR 
        Integer res = muni2Ver.recuperar(m);
        if(res == null){
            res = -1;
        }
        return res;
    ...
    public ListaConPI<Municipio> caminoMinimo(Municipio mOrig, Municipio mDst) {    
        ListaConPI<Integer> camino =  grafo.caminoMinimo(obtenerCodigo(mOrig), obtenerCodigo(mDst));
        ListaConPI<Municipio> res = new LEGListaConPI<Municipio>();
        // COMPLETAR
        for (camino.inicio(); !camino.esFin(); camino.siguiente()) {
            res.insertar(getMunicipio(camino.recuperar()));
        }
        return res;  
    }
}
