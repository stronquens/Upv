    ...
    */
    ...
    protected final void rehashing() {
        ...
        for(int j = 0; j< aux.length; j++){
            ListaConPI<EntradaHash<C, V>> l = aux[j];
            l.inicio();
            while(!l.esFin()){
                EntradaHash<C, V> elem = l.recuperar();    
                int indice = indiceHash(elem.clave);
                elArray[indice].insertar(elem); 
                l.siguiente();
            }
        }
    } 
    ...
    public ListaConPI<C> claves() {
        //COMPLETAR
        ListaConPI<C> lp = new LEGListaConPI<C>();
        for(int i=0; i<elArray.length;i++){
    ...
    public final double desviacionTipica() {
        //COMPLETAR
        double media, sumDesv = 0.0;
        int sum = 0;
        for (int i=0;i <elArray.length; i++){
            sum += elArray[i].talla();
        }
        media = (double) sum/elArray.length;
        for (int i=0;i <elArray.length; i++){
            sumDesv += Math.pow((elArray[i].talla() - media),2);
        }     
        return Math.sqrt((sumDesv/elArray.length));
    }
    ...
    public String histograma() {
        ...
        int talla;
        for(int i=0; i<elArray.length; i++){
            int aux = elArray[i].talla();
            if (aux < 9){
                lineas[aux] += 1;
            }else {
                lineas[lineas.length-1] += 1;
            }
