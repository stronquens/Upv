...
public abstract class Grafo {
    ...
    protected void dijkstra(int origen) {
        ...
        ColaPrioridad<Arista> cp = new LPIColaPrioridad<Arista>();

        cp.insertar(new Arista(origen,0));

        while (!cp.esVacia()){
            Arista ar = cp.eliminarMin();
            ...
            if( visitados[destino] == 0){
                ...
                ListaConPI<Adyacente> l = adyacentesDe(destino);
                for(l.inicio(); !l.esFin(); l.siguiente()){
                    int w = l.recuperar().destino;
                    double pesoW = l.recuperar().peso;

                    if(distanciaMin[w] > (distanciaMin[destino] + pesoW)){
                        distanciaMin[w] = distanciaMin[destino] + pesoW;
                        caminoMin[w] = destino;
                        cp.insertar(new Arista(w, distanciaMin[w]));
                    }
    ...
    public ListaConPI<Integer> caminoMinimo(int origen, int destino) {      
        ...
        int aux = destino;
        dijkstra(origen);
        while( aux > 0 && aux != origen){
            res.insertar(aux);
            res.inicio();
            aux = caminoMin[aux];
            if(aux == origen) hay=true;
        }
        res.insertar(origen);
        return (hay ? res : new LEGListaConPI<Integer>() );
    }   
